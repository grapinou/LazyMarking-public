...
func TestHybridPendingDisagreementCannotDownloadArtifacts(t *testing.T) {
	...
	if _, err := fixture.conn.Exec(`
		UPDATE marking_jobs
		SET review_policy_version='detector-agreement-v1', exam_name='corrected.pdf', mark_table_name='mark-table.pdf'
		WHERE id=50;
...
func TestHybridLastDisagreementReviewRegeneratesAndFinalizes(t *testing.T) {
	fixture := newReviewPageFixture(t)
	if _, err := fixture.conn.Exec(`
		UPDATE marking_jobs SET review_policy_version='detector-agreement-v1' WHERE id=50;
