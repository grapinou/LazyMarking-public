...
func AddFormQCMQuestionHandler(w http.ResponseWriter, r *http.Request, queries *db.Queries) {
	...
	dataPage := data.QCMQuestionPageData{
		...
		Selector: data.QCMQuestionSelectorData{
			FilterURL:            data.DefaultQCMQuestionRoutes.AddURL,
			SubmitURL:            addURL,
			TableURL:             tableURL,
			ResetURL:             addURL,
			Subjects:             subjects,
			...
			HasActiveFilters: subjectID.Valid || themeID.Valid || yearLevelID.Valid ||
				skillID.Valid || difficultyID.Valid || pointID.Valid,
		},
	}
